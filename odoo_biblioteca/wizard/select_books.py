...
class SelectBooksToRent(models.TransientModel):
    ...
    copy_ids = fields.Many2many('biblioteca.copy', string="Book copies", required=True)
    customer_id = fields.Many2one('res.partner', string="Customer")
    rental_ids = fields.Many2many('biblioteca.rental')
    ...
    def next_step(self):
        for copy in self.copy_ids:
            copy.rental_ids |= self.env['biblioteca.rental'].create({'copy_id': copy.id, 'customer_id': self.customer_id.id, 'return_date': self.return_date})
        return {
            'name':      'Rentals of %s' % (self.customer_id.name),
            'type':      'ir.actions.act_window',
            'res_model': 'biblioteca.rental',
            'view_mode': 'tree,form',
            'view_type': 'form',
